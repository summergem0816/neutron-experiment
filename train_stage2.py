...
def log_to_file(file_path, global_step, lr, loss=None, ckpt_path=""):
    with open(file_path, "a") as f:
        line = f"{global_step},{lr:.2e}"
        if loss is not None:
            line += f",{loss:.6f}"
        else:
            line += ", "
...
def main(args) -> None:
    ...
    if accelerator.is_main_process:
        ...
        # 初始化 train_log.txt 文件
        log_file = os.path.join(exp_dir, "train_log.txt")
        with open(log_file, "w") as f:
            f.write("global_step,lr,loss,ckpt_path\n")  # 写入表头
        ...
        for batch in loader:
            ...
            if global_step % cfg.train.log_every == 0 and global_step > 0:
                ...
                if accelerator.is_main_process:
                    ...
                    # 记录到 txt 文件
                    log_to_file(log_file, global_step, current_lr, loss=avg_loss)

            # Save checkpoint:
            if (global_step % cfg.train.ckpt_every == 0 or global_step in [10_000, 20_000]) and global_step > 0:
                if accelerator.is_main_process:
                    ...
                    avg_loss_for_log = (
                        accelerator.gather(torch.tensor(step_loss, device=device).unsqueeze(0))
                            .mean()
                            .item()
                    )
                    log_to_file(log_file, global_step, current_lr, loss=avg_loss_for_log, ckpt_path=ckpt_path)
